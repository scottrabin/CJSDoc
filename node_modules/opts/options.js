...
exports.loadConfiguration = function( file ){
	...
	exports.addOption( conf[i].shortname, conf[i].longname, conf[i].value, conf[i].description );
    }
};
...
exports.parse = function( args ){
    var opts = { "target": [] }, opt, match, optLen = options.length ;

    // loop through the args
    for( var i = 0, l = args.length ; i < l ; ++i ){
	// grab the option params
	match = args[i].match( opt_regex );
	// not an option? add it to the target
	if( match === null ){
	    opts.target.push( args[i] );
	}

	// get the option
	opt = ( match[1].length === 2 ? optsByLongname : optsByShortname )[ match[2] ];
	if( typeof opt === "object" ){
	    if( opt.value && args[i][0] !== '-' ){
		if( args[i][0] === '-' ){
		    // needs a value
		    throw new Error( "Option --" + opt.longname + " requires a value" );
		}
		opts[ opt.longname ] = args[i++];
	    } else {
		opts[ opt.longname ] = true;
	    }
	} else {
